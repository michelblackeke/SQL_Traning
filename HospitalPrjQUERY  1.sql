...
SELECT DISTINCT 
	CITY, 
	COUNT(ID) AS P
FROM Hospital_dev.dbo.PATIENTS
WHERE CITY <> 'QUINCY'
GROUP BY CITY
HAVING COUNT(ID) > 1
ORDER BY CITY DESC
...
WITH VISITS AS
	...
	FROM Hospital_dev.dbo.ENCOUNTERS A
		JOIN Hospital_dev.dbo.PATIENTS B 
		ON A.PATIENT=B.ID
...
WITH RECCURRINGCONDITIONS AS
	...
	FROM Hospital_dev.dbo.CONDITIONS A
		JOIN Hospital_dev.dbo.PATIENTS B 
		ON A.PATIENT=B.ID
...
FROM
	...
	FROM Hospital_dev.dbo.IMMUNIZATIONS A
		JOIN Hospital_dev.dbo.PATIENTS B 
		ON A.PATIENT=B.ID
	GROUP BY 
